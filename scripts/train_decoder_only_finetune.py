...
def build_parser():
    parser = argparse.ArgumentParser(description="Run Exp2 S1 decoder-only acoustic finetune.")
...
def main():
    ...
    if scope != DECODER_ONLY_SCOPE:
        raise ValueError(f"Expected finetune_scope={DECODER_ONLY_SCOPE}, got {scope!r}")

    checkpoint = cfg.get("pretrained_generator_checkpoint")
    if not checkpoint:
        raise ValueError("decoder-only finetune requires pretrained_generator_checkpoint in config")

    generator = build_model(cfg)
    load_generator_checkpoint(generator, checkpoint)
    freeze_report = apply_generator_train_scope(generator, scope)

    results_folder = Path(cfg["results_folder"])
    write_json(results_folder.parent / "reports" / "freeze_report.json", freeze_report)
