...
const stagingDir = path.join(buildDir, "editable-finalizer");
const finalPptx = path.join(workspaceDir, "figures", "ppt", "scit_speech_figure_blueprint_editable.pptx");
...
function paintFill(props) {
  ...
  return fill === "none" ? "none" : normalizeColor(fill);
}
...
function paintLine(props) {
  ...
  return {
    ...
    width: number(props["stroke-width"], 1),
  };
}
...
function addText(attrs, rawText, offset) {
  const props = computed(attrs);
  const content = decodeXml(rawText).replace(/\s+/gu, " ").trim();
  if (!content) return;
  const font = parseFont(props);
  const x = number(props.x) + offset.x;
  const y = number(props.y) + offset.y;
  const anchor = props["text-anchor"] ?? "start";
  const width = Math.max(font.size * 2.2, Math.min(950, content.length * font.size * 0.68 + 18));
  const height = Math.max(22, font.size * 1.55);
  let left = x;
  let alignment = "left";
  if (anchor === "middle") {
    left = x - width / 2;
  ...
  const top = props["dominant-baseline"] === "middle" ? y - height / 2 : y - font.size * 1.08;
...
slide.speakerNotes.textFrame.setText(
  "Reconstructed as native PowerPoint shapes from figures/scit_speech_figure_blueprint.svg. Every box, line, arrowhead, and text label is independently editable.",
);

const draftPath = path.join(stagingDir, "candidate-editable.pptx");
await (await PresentationFile.exportPptx(presentation)).save(draftPath);
const preview = await presentation.export({ slide, format: "png", scale: 1 });
await fs.writeFile(path.join(buildDir, "editable-draft-slide-1.png"), new Uint8Array(await preview.arrayBuffer()));
...
const result = await finalizePresentation({
  ...
  receiptPath: path.join(buildDir, "scit_speech_figure_blueprint_editable.validation.json"),
});

console.log(JSON.stringify({ finalPptx, draftPath, counts, totalObjects: Object.values(counts).reduce((a, b) => a + b, 0), result }, null, 2));
