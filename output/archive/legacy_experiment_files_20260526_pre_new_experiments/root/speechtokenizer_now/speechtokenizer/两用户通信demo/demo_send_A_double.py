...
def main():
    ...
    parser.add_argument("--peer_ip", required=True, help="Target host IP (e.g., 192.168.31.xx)")
    ...
    # 打印：准备连接的目标
    print(f"[TCP] connecting to {args.peer_ip}:{args.peer_port} ...")

    # 发起连接：如果 B 端没开服务/端口不通，会在这里报错
    # sock.connect((HOST, args.peer_port))
    sock.connect((args.peer_ip, args.peer_port))
