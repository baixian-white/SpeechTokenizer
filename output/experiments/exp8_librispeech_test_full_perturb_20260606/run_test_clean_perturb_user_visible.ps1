...
'USER_VISIBLE_STARTED ' + (Get-Date) | Set-Content 'H:\H-CODE\speechtokenizer\output\experiments\exp8_librispeech_test_full_perturb_20260606\logs\user_visible_marker.txt'
Start-Transcript -Path 'H:\H-CODE\speechtokenizer\output\experiments\exp8_librispeech_test_full_perturb_20260606\logs\test-clean_perturb_user_visible_transcript.log' -Append
Write-Host '=== SCIT perturb test-clean, user-visible PowerShell ==='
